...
difficult, as simulators rarely admit a tractable likelihood
...
We present results of the method with with non-differentiable simulators producing discrete/continuous univariate/multi-dimensional data
...
\begin{align}
   W( p_r(\mathbf{x} ), p(\mathbf{x} | \theta)  ) = \mathcal{L}_W = \sup_{f} \mathbb{E}_{p(\tilde{\mathbf{x}}|\theta)} [f(\tilde{\mathbf{x}})] - \mathbb{E}_{p_r(\mathbf{x})} [f(\mathbf{x})]
\end{align}
...
%\begin{equation}
%\psi^* = \inf_\psi \sup_\phi \mathbb{E}_{ \theta \sim q(\theta | \psi)} [ (O^{p,q} f_\phi) ]
%\end{equation}
%
%\begin{equation}
%(O^{p,q} f) = W(p,q) = \sup_f \mathbb{E}_{x\sim p}[f(x)] - \mathbb{E}_{x \sim q}[f(x)]
%\end{equation}
%The expectation should be $x \sim p(x|\psi)$ and here $(O^{p,q} f)$ is a scalar and not a function of $x$

To get into OPVI formalism

\begin{eqnarray}
\psi^* &=& \inf_\psi \sup_\phi\mathbb{E}_{x\sim p_r(x)}[f(x)] - \mathbb{E}_{x \sim p(x|\psi)}[f(x)] \\
&=&  \inf_\psi \sup_\phi \mathbb{E}_{ x \sim p( x | \psi)} [ (O^{p_r,p_\psi} f_\phi) ] 
\end{eqnarray}
with
\begin{equation}
(O^{p,q} f) = \left( \frac{p_r(x)}{p(x|\psi)}  -1 \right) f(x)
\end{equation}

We can think of $p(x|\psi)$ as a \textit{variational program} as described in ~\cite{opvi}, though more complicated than the a simple reparametrization of normally distributed noise $\epsilon$ through a differentiable function $R(\epsilon, \psi)$. In our case, the variational program is a marginalized non-differentiable simulator simulator.  Nevertheless, it can generate samples for $x$, is differentiable with respect to $\phi$, and its density is be intractable.
