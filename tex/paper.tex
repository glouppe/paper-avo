...
distribution $p(\mathbf{z}|\theta)$ may itself depend on $\theta$ in some intricated way.
...
\begin{figure*}
    ...
    \begin{algorithmic}[1]
        ...
        \While{$\psi$ has not converged}
            ...
            \EndFor
            \State{Sample a mini-batch $\{ \theta_m \sim q(\theta|\psi),  \mathbf{z}_m \sim p(\mathbf{z}|\theta_m) \}_{m=1}^M$.}  \Comment{Update $q_\psi$}
...
comparable to the latter. Closest to our work are procedures that rely on a
classifier to estimate the discrepancy between the true and the generated data
distributions. For example, \citep{gutmann2012noise} uses non linear logistic
regression for fitting unnormalized differentiable statistical models, while
...
\glnote{Explain what's new about the proposed work.}
