...
\glnote{contour plots of q(theta|psi) + histograms + learning curves}
\glnote{contours maybe with $\gamma=0$ and $\gamma>0$ contours}
\glnote{insist that we dont learn the simulator}

As a first illustrative experiment, we evaluate adversarial variational
optimization on a Poisson distribution with unknown scale parameter $\lambda$.
We artificially consider the distribution as a parameterized black box
simulator, from which we can only generate (discrete) data.
...
and the following configuration. For the critic, we use a 3-layer MLP with 10
...
each epoch in order to avoid building momentum in staled directions.  For
...
each epoch, parameters $\psi$ are updated by making one Adam step, with
$\alpha=0.01$ and $\beta_1=\beta_2=0.1$. Penalty coefficients are set to
$\lambda=0.025$ and $\gamma=5$.
...
\begin{figure}
    ...
    \vspace{-1cm}
    \caption{Toy problem on discrete data: Poisson with unknown scale.
             This example illustrates adversarial variational optimization for a discrete non-differentiable generator.
             (Top) Model distribution $p(\mathbf{x}|\psi)$ after adversarial variational optimization along with the proposal distribution $q(\lambda|\psi)$.
             (Bottom) Evolution of the variational upper bound $U_d$ on the loss ${\cal L}_d$ of the critic.
             }\label{fig:poisson}
\end{figure}

The upper plot of Figure~\ref{fig:poisson} illustrates the resulting model
after the optimization procedure. As theoretically expected from adversarial
training, the model density $p(\mathbf{x}|\psi)$ closely matches the true
density $p_r(\mathbf{x})$, as shown by the aligned histograms in blue and
orange. Under the effect of the entropy penalty $\gamma H(q(\theta|\psi))$, we
also observe that the proposal distribution has correctly concentrated its
mass around $\log(7) = 1.94$. The bottom plot of
Figure~\ref{fig:poisson} shows an empirical estimate of $-U_d$ with respect to
the epoch. We observe that it quickly falls towards $0$, which indicates that
...
between true and model data. Despite the discreteness and the non-differentiability
of the underlying generator, this confirms that inference with
adversarial variational optimization works.


\subsection{Multidimensional continuous data}

\glnote{use 5d example from carl}
\glnote{contour plots}
