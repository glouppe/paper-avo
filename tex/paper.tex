...
\begin{align}
   W( p_r(\mathbf{x} ), p(\mathbf{x} | \theta)  ) = \mathcal{L}_W = \sup_{f} \mathbb{E}_{p(\mathbf{x}|\theta)} [f(\tilde{\mathbf{x}})] - \mathbb{E}_{p_r(\mathbf{x})} [f(\mathbf{x})]
\end{align}
...
\glnote{Discuss 1702.08235 and references therein on adversarial variational bayes/inference.}
