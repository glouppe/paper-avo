...
autoencoders~\citep{DBLP:journals/corr/KingmaW13} and generative adversarial
...
dimensionality of $\bfz$ and the nature of its components (uniform, normal,
...
the dimensionality of $\bfz$ may be much larger than the dimensionality of
...
$\bfpsi$ through the lens of emperical Bayes, though the prior family
...
When $\rho$ is the Wasserstein distance, $\bftheta^*$ is refered to as the
...
use Kantorovich-Rubinstein duality and the critic $d$. For high-dimensional
...
variational program $q(\bfx | \bf\theta)$. This paired with the critic
Kantorovich-Rubinstein dual formulation of the Wasserstein distance using the
critic $d$ allows us to work in high dimensions without summary statistics and
...
This work has many connections to work on variational inference, in which the
goal is to optimize the recognition model $q(\bfz, \bftheta|\bfpsi)$ so that it
is close to the true posterior $p(\bfz, \bftheta |x)$. There have been efforts
