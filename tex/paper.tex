...
%\end{equation} 
\begin{equation}\label{eqn:grad-ug-approx}
   \nabla_\bfpsi U_g = \mathbb{E}_{\substack{\bftheta \sim q(\bftheta|\bfpsi), \\ \hat{\bfx} \sim p(\bfx | \bftheta)}}  [-d( \hat{\bfx} ;\bfphi) \nabla_\bfpsi \log q(\bftheta|\bfpsi)],
\end{equation} 
%\begin{equation}\label{eqn:grad-ug-approx}
...
\begin{equation}\label{eqn:p_psi}
    \mathbf{x} \sim \qxpsi \equiv \bftheta \sim q(\bftheta|\bfpsi), \bfz \sim p(\bfz|\bftheta), \mathbf{x} = g(\bfz; \bftheta).
\end{equation}
Effectively, we are optimizing a variational posterior $q(\bftheta | \psi)$ by minimizing the 
...
Consequently, the proposed algorithm is a form of variational Bayesian inference through model criticism~\citep{box1980sampling}. 
...
If instead, one is interested in point estimates $\bftheta^*$, then we augment Eqn.~\ref{eqn:vo-ug}
...
tightly around one or a few $\bftheta$ values. Very large penalties may 
