...
Moreover, the algorithm could be adopted to use estimates to the KL distance~\citep{perez2009estimation} without the adversarial optimization for the critic. These variants may be particularly useful for small $N$.
...
Effectively, we are optimizing the variational posterior $q(\bftheta | \bfpsi)$ by
...
where one specifies a family of priors and $p(\bftheta | \bfpsi)$ and then estimates $\bfpsi^*$ using the data.
...
When $M_g \gg M_r$, then the variance of $\bftheta^*$ scales ad $\sqrt{M_r}$~\citep{bernton2017inference}. A final $\sqrt{N/M}$ scaling to the log-posterior $q(\bftheta | \bfpsi)$ is needed to reflect the full strength of the data.
...
\end{eqnarray}
In traditional VI with the KL distance, we have $(O^{p,q} f) = \log q(\bfz) - \log p(\bfz|\bfx) \, \forall f \in \mathcal{F}$.
AVO can be cast into a similar form with expectations over $\bfx$ instead of $\bfz$ and
\begin{eqnarray} \label{eq:opvi_avo}
...
The AVO-EB algorithm enables empirical Bayes through variational inference in the likelihood-free setting.
