...
\begin{align}
    {\cal L}_d =\,&  \mathcal{L}_W + \lambda_{GP} \mathbb{E}_{\hat{\mathbf{x}} \sim p({\hat{\mathbf{x}})}} [(|| \nabla_{\hat{\mathbf{x}}} d({\hat{\mathbf{x}}};\bfphi) ||_2 - 1)^2] \\
...
\begin{figure*}
    ...
    \begin{flushleft}
        ...
        {\it Hyper-parameters:} The number $n_{\text{critic}}$ of training iterations of $d$; the size $M$ of a mini-batch; the gradient penalty coefficient $\lambda_{GP}$; the entropy penalty coefficient $\gamma$.
    \end{flushleft}
        ...
        \While{$\bfpsi$ has not converged}
            \For{$i=1$ to $n_{\text{critic}}$} \Comment{Update $d$}
                ...
                \For{$m=1$ to $M$}
                    ...
                    \State{$U_d^{(m)} \leftarrow d(\tilde{\mathbf{x}}_m; \bfphi) - d(\mathbf{x}_m; \bfphi) + \lambda_{GP} (|| \nabla_{\hat{\mathbf{x}}_m} d(\hat{\mathbf{x}}_m; \bfphi) ||_2 - 1)^2 $}
                \EndFor
...
$\lambda_{GP}=0.001$, and the entropy penalty is evaluated at both $\gamma=0$ and $\gamma=3$.
...
Hyper-parameters are set to $M=50$, $n_\text{critic}=100$, $\lambda_{GP}=0.01$, $\gamma=1$ and
...
$\beta_1=0.5$ and $\beta_2=0.9$. For the critic $d$, we use a 3-layer MLP with 50
hidden nodes per layer and ReLU activations.
...
in the neighborhood of $\bftheta^*$.
Despite the width of the resulting proposal distribution with respect to $\bftheta^*$,
the synthetic data closely match with the observed data, as shown
...
reproduced through a different predictive distribution $\qxpsi$.
