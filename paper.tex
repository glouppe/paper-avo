...
marginal distribution  matches the observed data.
...
non-differentiable minimax problem by minimizing variational upper bounds of the adversarial objectives
respectively with stochastic gradient descent and  natural evolution strategies~\citep{2011arXiv1106.4487W}.
The procedure results in learning a proposal distribution over
...
\begin{figure*}
    ...
    \begin{algorithmic}[1]
        ...
        \While{$\psi$ has not converged}
            ...
            \State{$\psi \leftarrow \text{SGD}(\mathbf{F}(\psi)^{-1} \nabla_\psi U_g + \gamma \nabla_\psi H(q_\psi)$}
        \EndWhile
    \end{algorithmic}
    \end{algorithm}
    \end{minipage}
...
In this work, we propose to rely on evolution strategies to minimize ${\cal
...
optimization procedure, as built on top of WGAN-GP and NES.
