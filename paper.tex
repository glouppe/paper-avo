...
\begin{figure*}
    ...
    \begin{flushleft}
        {\it Inputs:} observed data $\{ \mathbf{x}_i \}_{i=1}^N$, simulator $g$;\\
        {\it Outputs:} proposal distribution $q_\psi(\theta)$;\\
        {\it Hyper-parameters:} Number $T$ of training iterations,
                                Number $K$ of gradient steps to update $d$,
...
For completeness, Algorithm~\ref{alg:avo} finally outlines the adversarial variational
optimization procedure when coupled with WGAN-GP.
